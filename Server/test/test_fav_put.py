...
def test_fav_put_invalid_token(mock1):
    response = app.test_client().put(
        '/fav',
...
@patch("src.database.db.new_fav", side_effect=db.NoSuchActivityException())
def test_fav_put_wrong_name(mock1, mock2):
    response = app.test_client().put(
        '/fav',
        query_string = {'token' : json.dumps(fake_token), 'name' : 'test'}
        )
...
@patch("src.googleservice.gs.token_to_creds_json_and_email", return_value=('creds', 'a@a.com'))
@patch("src.database.db.new_fav", side_effect=Exception())
def test_fav_put_database_error(mock1, mock2):
    response = app.test_client().put(
        '/fav',
        query_string = {'token' : json.dumps(fake_token), 'name' : 'test'}
        )
...
@patch("src.googleservice.gs.token_to_creds_json_and_email", return_value=('creds', 'a@a.com'))
@patch("src.database.db.new_fav", return_value=None)
